...
public class ECKeyTemplateSCTest extends TestCase{

    
    static SmartCard SC = null;
    static String PASSWORD = null;
    static long SLOT = -1;
    final static String  CREATED_KEY_LABEL = "CreatedECKey"+System.currentTimeMillis();
    final static String  IMPORTED_KEY_LABEL = "ImportedECKey"+System.currentTimeMillis();
    ...
    public static Pair<Pair<SmartCard,Long>,List<ECertificate>> readCertForEnc(int count) throws Exception {
        ...
        long sessionID =-1;
        List<ECertificate> retCertList = null;
        for (int i = 0; i < count; i++) {
            CardType cardType = CardType.UTIMACO;
            retCertList = new ArrayList<ECertificate>();
            ...
            if(encryptionCertificates.size()>0){
                retCertList.add(new ECertificate(encryptionCertificates.get(0)));
            }
        }
        return new Pair<Pair<SmartCard, Long>, List<ECertificate>>(new Pair<SmartCard, Long>(smartCard,sessionID),retCertList);
    }


    //prime field
    public void testCreateECKeyPair_secp160r1_RMZ()
            throws Exception {
        SmartCard smartCard=null;
        long sessionId=-1;
        try{

            Pair<Pair<SmartCard, Long>, List<ECertificate>> smartCardInfo = readCertForEnc(1);
            Pair<SmartCard, Long> smartCardSession = smartCardInfo.getObject1();
            smartCard = smartCardSession.first();
            sessionId = smartCardSession.getObject2();


            String curveName = "secp160r1";
            ECGenParameterSpec spec = new ECGenParameterSpec(curveName);

            String keyLabel = curveName+"_"+CREATED_KEY_LABEL;

			ECParameterSpec ecParameterSpec = NamedCurve.getECParameterSpec(curveName);
			ECPublicKeyTemplate ecPublicKeyTemplate=new ECPublicKeyTemplate(keyLabel,ecParameterSpec);
            ECPrivateKeyTemplate ecPrivateKeyTemplate=new ECPrivateKeyTemplate(keyLabel,ecParameterSpec);

            ECKeyPairTemplate ecKeyPairTemplate = new ECKeyPairTemplate(ecPublicKeyTemplate,ecPrivateKeyTemplate);
            ecKeyPairTemplate.getAsSignerAndVerifierTemplate();
            //ecKeyPairTemplate.getAsExtractableTemplate();
            ecKeyPairTemplate.getAsTokenOrSessionTemplate(false);
            //ecKeyPairTemplate = ecKeyPairTemplate.getAsTokenTemplate(true,false);

            KeySpec ecKeyPairSpec = smartCard.createKeyPair(sessionId, ecKeyPairTemplate);

            boolean sonuc1 = smartCard.isPrivateKeyExist(sessionId, keyLabel);
            boolean sonuc2 = smartCard.isPublicKeyExist(sessionId, keyLabel);
            ECPublicKeySpec readSpec = (ECPublicKeySpec) smartCard.readPublicKeySpec(sessionId, keyLabel);
   ...
   /* public void testImportECKeyPair_secp521r1()
    ...
    public void testSignEC384()
    throws Exception
    {

		ECertificate eCertificate = ECertificate.readFromFile("G:\\tmp\\caCertificate.cer");

        SLOT = 43;
        long sid = SC.openSession(SLOT);
        SC.login(sid, PASSWORD);
        String keyName = "kokSignv1";
        byte[] signature = SC.signData(sid, keyName, "test".getBytes(), PKCS11Constants.CKM_ECDSA_SHA1);
        ECPublicKeySpec spec = (ECPublicKeySpec) SC.readPublicKeySpec(sid, keyName);
        boolean scSonuc = true;
        try
	...
	try
	{
	    ECKeyTemplateSCTest test = new ECKeyTemplateSCTest();
