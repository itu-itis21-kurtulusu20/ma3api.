 ...
 */
public class AtikHSMUsageTests extends BaseCardTest {

    @BeforeClass
    public static void setUp() throws Exception {
        BaseCardTest.cardType = CardType.ATIKHSM;
        BaseCardTest.slotNo = 1;
        BaseCardTest.slotPin="1234";
        BaseCardTest.setUp();
    }
    ...
    private List<String> importKeyAndCertificate(int count, boolean addKeysAndCertsToDeleteList, boolean importCertificate, boolean importKey) throws ESYAException, IOException, PKCS11Exception {
        ...
        while (importedCount < count) {
            ...
            for (Pair<ECertificate, PrivateKey> certificateAndKey : certificatesAndKeys) {
                ...
                byte[] certBytes = AsnIO.dosyadanOKU("G:\\tmp/adminCert.cer");
                //byte[] certBytes = AsnIO.dosyadanOKU("G:\\tmp/ramazang#uekaeSIGN0.cer");
    ...
    private void signAndVerifyWithKeyLabelPKCS15(String signKeyLabel, PublicKey publicKey) throws PKCS11Exception, SmartCardException, CryptoException {
        ...
        byte[] signature =  SmartOp.sign(sc, sessionId, slotNo, signKeyLabel, dataForSign, SignatureAlg.RSA_SHA256.getName());
    ...
    public void testSignWithSmartCardAndVerifyWithMemoryPKCS() throws PKCS11Exception, ESYAException {
        String[] signatureKeyLabels = sc.getSignatureKeyLabels(sessionId);
        String signKeyLabel = "CASIGN2048";//signatureKeyLabels[0];
        KeySpec keySpec = sc.readPublicKeySpec(sessionId, signKeyLabel);
        PublicKey publicKey = KeyUtil.generatePublicKey(keySpec);
    ...
    public void testEncryptWithMemoryAndDecryptWithSmartCardPKCS15() throws PKCS11Exception, ESYAException, IOException {
        String[] encryptionKeyLabels = sc.getEncryptionKeyLabels(sessionId);
        String encryptionKeyLabel ="CAENC2048";//encryptionKeyLabels[0];

        KeySpec encKeySpec = sc.readPublicKeySpec(sessionId, encryptionKeyLabel);
        PublicKey publicKey = KeyUtil.generatePublicKey(encKeySpec);
