...
public class Providers {
    ...
    private static final ThreadLocal<sune.security.jca.ProviderList> threadLists =
        new InheritableThreadLocal<sune.security.jca.ProviderList>();
    ...
    private static volatile sune.security.jca.ProviderList providerList;

    static {
        // set providerList to empty list first in case initialization somehow
        // triggers a getInstance() call (although that should not happen)
        providerList = sune.security.jca.ProviderList.EMPTY;
        providerList = sune.security.jca.ProviderList.fromSecurityProperties();
    }
    ...
    public static Object startJarVerification() {
        sune.security.jca.ProviderList currentList = getProviderList();
        sune.security.jca.ProviderList jarList = currentList.getJarList(jarVerificationProviders);
    ...
    public static void stopJarVerification(Object obj) {
        ...
        endThreadProviderList((sune.security.jca.ProviderList)obj);
    }
     ...
     */
    public static sune.security.jca.ProviderList getProviderList() {
        sune.security.jca.ProviderList list = getThreadProviderList();
        if (list == null) {
     ...
     */
    public static void setProviderList(sune.security.jca.ProviderList newList) {
     ...
     */
    public static synchronized sune.security.jca.ProviderList getFullProviderList() {
        sune.security.jca.ProviderList list = getThreadProviderList();
        if (list != null) {
            sune.security.jca.ProviderList newList = list.removeInvalid();
            if (newList != list) {
                changeThreadProviderList(newList);
                list = newList;
            }
    ...
    private static sune.security.jca.ProviderList getSystemProviderList() {
        return providerList;
    }

    private static void setSystemProviderList(sune.security.jca.ProviderList list) {
        providerList = list;
    }

    public static sune.security.jca.ProviderList getThreadProviderList() {
    ...
    private static void changeThreadProviderList(sune.security.jca.ProviderList list) {
    ...
    public static synchronized sune.security.jca.ProviderList beginThreadProviderList(sune.security.jca.ProviderList list) {
        if (sune.security.jca.ProviderList.debug != null) {
            sune.security.jca.ProviderList.debug.println("ThreadLocal providers: " + list);
        }
        sune.security.jca.ProviderList oldList = threadLists.get();
        threadListsUsed++;
        threadLists.set(list);
        return oldList;
    }

    public static synchronized void endThreadProviderList(sune.security.jca.ProviderList list) {
        if (list == null) {
            if (sune.security.jca.ProviderList.debug != null) {
                sune.security.jca.ProviderList.debug.println("Disabling ThreadLocal providers");
            }
            threadLists.remove();
        } else {
            if (sune.security.jca.ProviderList.debug != null) {
