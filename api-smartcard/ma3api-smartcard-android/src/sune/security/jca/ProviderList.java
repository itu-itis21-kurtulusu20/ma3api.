...
public final class ProviderList {
    ...
    private final static sune.security.jca.ProviderConfig[] PC0 = new sune.security.jca.ProviderConfig[0];
    ...
    public static ProviderList insertAt(ProviderList providerList, Provider p,
        ...
        List<sune.security.jca.ProviderConfig> list = new ArrayList<sune.security.jca.ProviderConfig>
                                    (Arrays.asList(providerList.configs));
        int n = list.size();
        if ((position < 0) || (position > n)) {
            position = n;
        }
        list.add(position, new sune.security.jca.ProviderConfig(p));
    ...
    public static ProviderList remove(ProviderList providerList, String name) {
        ...
        sune.security.jca.ProviderConfig[] configs = new sune.security.jca.ProviderConfig[providerList.size() - 1];
        int j = 0;
        for (sune.security.jca.ProviderConfig config : providerList.configs) {
    ...
    public static ProviderList newList(Provider ... providers) {
        sune.security.jca.ProviderConfig[] configs = new sune.security.jca.ProviderConfig[providers.length];
        for (int i = 0; i < providers.length; i++) {
            configs[i] = new sune.security.jca.ProviderConfig(providers[i]);
        }
        return new ProviderList(configs, true);
    }

    // configuration of the providers
    private final sune.security.jca.ProviderConfig[] configs;
     ...
     */
    private ProviderList(sune.security.jca.ProviderConfig[] configs, boolean allLoaded) {
    ...
    private ProviderList() {
        List<sune.security.jca.ProviderConfig> configList = new ArrayList<sune.security.jca.ProviderConfig>();
        for (int i = 1; true; i++) {
            ...
            int k = entry.indexOf(' ');
            sune.security.jca.ProviderConfig config;
            if (k == -1) {
                config = new sune.security.jca.ProviderConfig(entry);
            } else {
                String className = entry.substring(0, k);
                String argument = entry.substring(k + 1).trim();
                config = new sune.security.jca.ProviderConfig(className, argument);
            }
    ...
    ProviderList getJarList(String[] jarClassNames) {
        List<sune.security.jca.ProviderConfig> newConfigs = new ArrayList<sune.security.jca.ProviderConfig>();
        for (String className : jarClassNames) {
            sune.security.jca.ProviderConfig newConfig = new sune.security.jca.ProviderConfig(className);
            for (sune.security.jca.ProviderConfig config : configs) {
        ...
        sune.security.jca.ProviderConfig[] configArray = newConfigs.toArray(PC0);
    ...
    private sune.security.jca.ProviderConfig getProviderConfig(String name) {
    ...
    public Provider getProvider(String name) {
        sune.security.jca.ProviderConfig config = getProviderConfig(name);
    ...
    ProviderList removeInvalid() {
        ...
        sune.security.jca.ProviderConfig[] newConfigs = new sune.security.jca.ProviderConfig[n];
        for (int i = 0, j = 0; i < configs.length; i++) {
            sune.security.jca.ProviderConfig config = configs[i];
            if (config.isLoaded()) {
                newConfigs[j++] = config;
            }
        }
     ...
     * @deprecated use getServices(List{@literal <ServiceId>}) instead
     */
    @Deprecated
    public List<Service> getServices(String type, List<String> algorithms) {
        List<sune.security.jca.ServiceId> ids = new ArrayList<sune.security.jca.ServiceId>();
        for (String alg : algorithms) {
            ids.add(new sune.security.jca.ServiceId(type, alg));
        }
        return getServices(ids);
    }

    public List<Service> getServices(List<sune.security.jca.ServiceId> ids) {
    ...
    private final class ServiceList extends AbstractList<Service> {
        ...
        private final List<sune.security.jca.ServiceId> ids;
        ...
        ServiceList(List<sune.security.jca.ServiceId> ids) {
            this.type = null;
            this.algorithm = null;
            this.ids = ids;
        }
