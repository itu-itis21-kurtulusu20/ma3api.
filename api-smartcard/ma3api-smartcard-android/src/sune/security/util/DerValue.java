...
public class DerValue {
    ...
    protected sune.security.util.DerInputBuffer buffer;

    /**
     * The DER-encoded data of the value.
     */
    public final sune.security.util.DerInputStream data;
    ...
    public DerValue(byte tag, byte[] data) {
        this.tag = tag;
        buffer = new sune.security.util.DerInputBuffer(data.clone());
        length = data.length;
        this.data = new sune.security.util.DerInputStream(buffer);
     ...
     * package private
     */
    DerValue(sune.security.util.DerInputBuffer in) throws IOException {
        // XXX must also parse BER-encoded constructed
        // values such as sequences, sets...

        tag = (byte)in.read();
        byte lenByte = (byte)in.read();
        length = sune.security.util.DerInputStream.getLength((lenByte & 0xff), in);
        if (length == -1) {  // indefinite length encoding found
            sune.security.util.DerInputBuffer inbuf = in.dup();
            ...
            inbuf = new sune.security.util.DerInputBuffer(derIn.convert(indefData));
            if (tag != inbuf.read())
                throw new IOException
                        ("Indefinite length encoding not supported");
            length = sune.security.util.DerInputStream.getLength(inbuf);
            buffer = inbuf.dup();
            buffer.truncate(length);
            data = new sune.security.util.DerInputStream(buffer);
    ...
    private sune.security.util.DerInputStream init(byte stringTag, String value) throws IOException {
        String enc = null;
        ...
        length = buf.length;
        buffer = new sune.security.util.DerInputBuffer(buf);
        sune.security.util.DerInputStream result = new sune.security.util.DerInputStream(buffer);
     ...
     * helper routine
     */
    private sune.security.util.DerInputStream init(boolean fullyBuffered, InputStream in)
            throws IOException {
        ...
        if (length == -1) { // indefinite length encoding found
            ...
            if (tag != in.read())
                throw new IOException
                        ("Indefinite length encoding not supported");
            length = sune.security.util.DerInputStream.getLength(in);
        }
        if (length == 0)
            return null;
        ...
        dis.readFully(bytes);
        buffer = new sune.security.util.DerInputBuffer(bytes);
        return new sune.security.util.DerInputStream(buffer);
    }

    /**
     * Encode an ASN1/DER encoded datum onto a DER output stream.
     */
    public void encode(sune.security.util.DerOutputStream out)
    throws IOException {
        out.write(tag);
    ...
    public final sune.security.util.DerInputStream getData() {
        return data;
    }
    ...
    public byte[] getOctetString() throws IOException {
        ...
        if (isConstructed()) {
            sune.security.util.DerInputStream in = new sune.security.util.DerInputStream(bytes);
            bytes = null;
    ...
    public byte[] toByteArray() throws IOException {
        sune.security.util.DerOutputStream out = new DerOutputStream();
     ...
     */
    public sune.security.util.DerInputStream toDerInputStream() throws IOException {
