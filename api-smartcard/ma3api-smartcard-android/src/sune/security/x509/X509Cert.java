...
public class X509Cert implements Certificate, Serializable {
    ...
    /* The algorithm id */
    transient protected sune.security.x509.AlgorithmId algid;
     ...
     * X509.v1 data (parsed)
     */
    transient private sune.security.x509.X500Name subject; // from subject
    ...
    transient private sune.security.x509.X500Name issuer;
    transient private sune.security.x509.AlgorithmId issuerSigAlg;
    ...
    {
        sune.security.util.DerValue in = new sune.security.util.DerValue(cert);
        parse (in);
        if (in.data.available () != 0)
            throw new sune.security.x509.CertParseError("garbage at end");
        signedCert = cert;
    }
    ...
    {
        sune.security.util.DerValue in = new sune.security.util.DerValue(buf, offset, len);
     ...
     * @exception IOException when the certificate is improperly encoded.
     */
    public X509Cert(sune.security.util.DerValue derVal) throws IOException
    {
        parse (derVal);
     ...
     *
     * @exception sune.security.x509.CertException if the public key is inappropriate
     */
    public X509Cert(sune.security.x509.X500Name subjectName, sune.security.x509.X509Key subjectPublicKey,
                    Date notBefore, Date notAfter) throws sune.security.x509.CertException
    {
        subject = subjectName;

        if (!(subjectPublicKey instanceof PublicKey))
            throw new sune.security.x509.CertException(sune.security.x509.CertException.err_INVALID_PUBLIC_KEY,
     ...
     * Other code must manage and use those cert chains.</em><p>
     *
     * <p><em>For now, you must walk the cert chain being used to verify any
     ...
     * @exception sune.security.x509.CertException when the certificate is not valid.
     */
    public void verify (PublicKey issuerPublicKey)
    throws sune.security.x509.CertException
    {
        Date    now = new Date ();

        if (now.before (notbefore))
            throw new sune.security.x509.CertException(sune.security.x509.CertException.verf_INVALID_NOTBEFORE);
        if (now.after (notafter))
            throw new sune.security.x509.CertException(sune.security.x509.CertException.verf_INVALID_EXPIRED);
        if (signedCert == null)
            throw new sune.security.x509.CertException(sune.security.x509.CertException.verf_INVALID_SIG,
        ...
        // Gag -- too many catch clauses, let most through.
            ...
            throw new sune.security.x509.CertException(CertException.verf_INVALID_SIG,
                "Signature by <" + issuer + "> for <" + subject + ">");
        }
    }
     ...
     *  signature algorithm
     */
    public X500Signer   getSigner (sune.security.x509.AlgorithmId algorithmId,
     ...
     */
    public sune.security.x509.X500Name getSubjectName ()
        { return subject; }
     ...
     * Null is returned in the case of a partially constructed cert.
     */
    public sune.security.x509.X500Name getIssuerName ()
        { return issuer; }
     ...
     * Null is returned in the case of a partially constructed cert.
     */
    public sune.security.x509.AlgorithmId getIssuerAlgorithmId ()
        { return issuerSigAlg; }
    ...
    public String       toString ()
    {
        ...
        s += "  Serial number = " + sune.security.util.Debug.toHexString(serialnum) + "\n";
     ...
     */
    private void parse (sune.security.util.DerValue val) throws IOException
    {
        if (parsed == true) {
            throw new IOException("Certificate already parsed");
        }

        sune.security.util.DerValue seq [] = new sune.security.util.DerValue[3];
        ...
        if (val.data.available () != 0)
            throw new sune.security.x509.CertParseError("signed overrun, bytes = "
                    + val.data.available ());
        if (seq [0].tag != sune.security.util.DerValue.tag_Sequence)
            throw new sune.security.x509.CertParseError("signed fields invalid");

        rawCert = seq [0].toByteArray ();       // XXX slow; fixme!


        issuerSigAlg = sune.security.x509.AlgorithmId.parse (seq [1]);
        signature = seq [2].getBitString ();

        if (seq [1].data.available () != 0) {
            // XXX why was this error check commented out?
            // It was originally part of the next check.
            throw new sune.security.x509.CertParseError("algid field overrun");
        }

        if (seq [2].data.available () != 0)
            throw new sune.security.x509.CertParseError("signed fields overrun");

        /*
         * Let's have fun parsing the cert itself.
         */
        sune.security.util.DerInputStream in;
        sune.security.util.DerValue tmp;
        ...
        tmp = in.getDerValue ();
        {
            sune.security.x509.AlgorithmId algid;


            algid = AlgorithmId.parse(tmp);

            if (!algid.equals (issuerSigAlg))
                throw new sune.security.x509.CertParseError("CA Algorithm mismatch!");
         ...
         * issuer name
         */
        issuer = new sune.security.x509.X500Name(in);

        /*
         * validity:  SEQUENCE { start date, end date }
         */
        tmp = in.getDerValue ();
        if (tmp.tag != sune.security.util.DerValue.tag_Sequence)
            throw new sune.security.x509.CertParseError("corrupt validity field");

        notbefore = tmp.data.getUTCTime ();
        notafter = tmp.data.getUTCTime ();
        if (tmp.data.available () != 0)
            throw new sune.security.x509.CertParseError("excess validity data");
    ...
    {
        sune.security.util.DerOutputStream raw = new sune.security.util.DerOutputStream();
     ...
     */
    private void encode (sune.security.util.DerOutputStream out) throws IOException
    {
