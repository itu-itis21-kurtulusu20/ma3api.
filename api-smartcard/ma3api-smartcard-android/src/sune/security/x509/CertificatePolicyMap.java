...
public class CertificatePolicyMap {
    private sune.security.x509.CertificatePolicyId issuerDomain;
    private sune.security.x509.CertificatePolicyId subjectDomain;
     ...
     */
    public CertificatePolicyMap(sune.security.x509.CertificatePolicyId issuer,
                                sune.security.x509.CertificatePolicyId subject) {
     ...
     */
    public CertificatePolicyMap(sune.security.util.DerValue val) throws IOException {
        if (val.tag != sune.security.util.DerValue.tag_Sequence) {
            throw new IOException("Invalid encoding for CertificatePolicyMap");
        }
        issuerDomain = new sune.security.x509.CertificatePolicyId(val.data.getDerValue());
        subjectDomain = new sune.security.x509.CertificatePolicyId(val.data.getDerValue());
    }

    /**
     * Return the issuer CA part of the policy map.
     */
    public sune.security.x509.CertificatePolicyId getIssuerIdentifier() {
        return (issuerDomain);
    }
