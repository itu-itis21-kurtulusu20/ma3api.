...
public class CertificatePolicySet {
    ...
    private final Vector<sune.security.x509.CertificatePolicyId> ids;
     ...
     */
    public CertificatePolicySet(Vector<sune.security.x509.CertificatePolicyId> ids) {
        this.ids = ids;
    }
     ...
     */
    public CertificatePolicySet(sune.security.util.DerInputStream in) throws IOException {
        ids = new Vector<sune.security.x509.CertificatePolicyId>();
        sune.security.util.DerValue[] seq = in.getSequence(5);

        for (int i = 0; i < seq.length; i++) {
            sune.security.x509.CertificatePolicyId id = new sune.security.x509.CertificatePolicyId(seq[i]);
            ids.addElement(id);
        }
    }
    ...
    public void encode(sune.security.util.DerOutputStream out) throws IOException {
        ...
        for (int i = 0; i < ids.size(); i++) {
            ((sune.security.x509.CertificatePolicyId)ids.elementAt(i)).encode(tmp);
        }
