...
implements CertAttrSet<String>, Cloneable {
    ...
    private sune.security.x509.GeneralSubtrees permitted = null;
    private sune.security.x509.GeneralSubtrees excluded = null;
    ...
    private void calcMinMax() throws IOException {
        ...
        if (excluded != null) {
            for (int i = 0; i < excluded.size(); i++) {
                sune.security.x509.GeneralSubtree subtree = excluded.get(i);
        ...
        if (permitted != null) {
            for (int i = 0; i < permitted.size(); i++) {
                sune.security.x509.GeneralSubtree subtree = permitted.get(i);
                if (subtree.getMinimum() != 0)
                    hasMin = true;
                if (subtree.getMaximum() != -1)
                    hasMax = true;
    ...
    private void encodeThis() throws IOException {
        ...
        if (permitted != null) {
            ...
            tagged.writeImplicit(DerValue.createTag(sune.security.util.DerValue.TAG_CONTEXT,
     ...
     */
    public NameConstraintsExtension(sune.security.x509.GeneralSubtrees permitted,
                                    sune.security.x509.GeneralSubtrees excluded)
    throws IOException {
        this.permitted = permitted;
        this.excluded = excluded;

        this.extensionId = sune.security.x509.PKIXExtensions.NameConstraints_Id;
        this.critical = true;
        encodeThis();
    }
    ...
    throws IOException {
        ...
        while (val.data.available() != 0) {
            ...
            if (opt.isContextSpecific(TAG_PERMITTED) && opt.isConstructed()) {
                ...
                opt.resetTag(sune.security.util.DerValue.tag_Sequence);
                permitted = new sune.security.x509.GeneralSubtrees(opt);
                ...
                opt.resetTag(sune.security.util.DerValue.tag_Sequence);
                excluded = new sune.security.x509.GeneralSubtrees(opt);
            } else
    ...
    public void set(String name, Object obj) throws IOException {
        if (name.equalsIgnoreCase(PERMITTED_SUBTREES)) {
            if (!(obj instanceof sune.security.x509.GeneralSubtrees)) {
                throw new IOException("Attribute value should be"
                                    + " of type GeneralSubtrees.");
            }
            permitted = (sune.security.x509.GeneralSubtrees)obj;
        } else if (name.equalsIgnoreCase(EXCLUDED_SUBTREES)) {
            if (!(obj instanceof sune.security.x509.GeneralSubtrees)) {
                throw new IOException("Attribute value should be "
                                    + "of type GeneralSubtrees.");
            }
            excluded = (sune.security.x509.GeneralSubtrees)obj;
        } else {
    ...
    public void merge(NameConstraintsExtension newConstraints)
        ...
        sune.security.x509.GeneralSubtrees newExcluded =
                        (sune.security.x509.GeneralSubtrees)newConstraints.get(EXCLUDED_SUBTREES);
        if (excluded == null) {
            excluded = (newExcluded != null) ?
                        (sune.security.x509.GeneralSubtrees)newExcluded.clone() : null;
        } else {
            if (newExcluded != null) {
        ...
        sune.security.x509.GeneralSubtrees newPermitted =
                (sune.security.x509.GeneralSubtrees)newConstraints.get(PERMITTED_SUBTREES);
        if (permitted == null) {
            permitted = (newPermitted != null) ?
                        (sune.security.x509.GeneralSubtrees)newPermitted.clone() : null;
        } else {
            if (newPermitted != null) {
                ...
                if (newExcluded != null) {
                    if (excluded != null) {
                        ...
                        excluded = (sune.security.x509.GeneralSubtrees)newExcluded.clone();
                    }
                }
            }
        }
    ...
    public boolean verify(X509Certificate cert) throws IOException {
        ...
        sune.security.x509.X500Name subject = sune.security.x509.X500Name.asX500Name(subjectPrincipal);
        ...
        sune.security.x509.GeneralNames altNames = null;
        // extract altNames
        try {
            // extract extensions, if any, from certInfo
            // following returns null if certificate contains no extensions
            sune.security.x509.X509CertImpl certImpl = X509CertImpl.toImpl(cert);
            ...
            throw new IOException("Unable to extract extensions from " + "certificate: " + ce.getMessage(), ce);
        }
        ...
        for (int i = 0; i < altNames.size(); i++) {
            sune.security.x509.GeneralNameInterface altGNI = altNames.get(i).getName();
            if (!verify(altGNI)) {
                return false;
            }
        }
     ...
     * @throws IOException on error
     */
    public boolean verify(sune.security.x509.GeneralNameInterface name) throws IOException {
        if (name == null) {
        ...
        if (excluded != null && excluded.size() > 0) {
            ...
            for (int i = 0; i < excluded.size(); i++) {
                sune.security.x509.GeneralSubtree gs = excluded.get(i);
                if (gs == null)
                    continue;
                sune.security.x509.GeneralName gn = gs.getName();
                if (gn == null)
                    continue;
                sune.security.x509.GeneralNameInterface exName = gn.getName();
                if (exName == null)
                    continue;
                ...
                case sune.security.x509.GeneralNameInterface.NAME_WIDENS: // name widens excluded
                case sune.security.x509.GeneralNameInterface.NAME_SAME_TYPE:
                    break;
                case sune.security.x509.GeneralNameInterface.NAME_MATCH:
                case sune.security.x509.GeneralNameInterface.NAME_NARROWS: // subject name excluded
                    return false;
                }
            }
        }
        ...
        if (permitted != null && permitted.size() > 0) {
            ...
            for (int i = 0; i < permitted.size(); i++) {
                ...
                if (gn == null)
                    continue;
                sune.security.x509.GeneralNameInterface perName = gn.getName();
                if (perName == null)
                    continue;
                ...
                case sune.security.x509.GeneralNameInterface.NAME_WIDENS: // name widens permitted
                case sune.security.x509.GeneralNameInterface.NAME_SAME_TYPE:
                    sameType = true;
    ...
    public boolean verifyRFC822SpecialCase(X500Name subject) throws IOException {
        for (Iterator t = subject.allAvas().iterator(); t.hasNext(); ) {
            sune.security.x509.AVA ava = (AVA)t.next();
            sune.security.util.ObjectIdentifier attrOID = ava.getObjectIdentifier();
            if (attrOID.equals(PKCS9Attribute.EMAIL_ADDRESS_OID)) {
                String attrValue = ava.getValueString();
                if (attrValue != null) {
                    sune.security.x509.RFC822Name emailName;
                    try {
    ...
    public Object clone() {
        try {
            ...
            if (permitted != null) {
                newNCE.permitted = (sune.security.x509.GeneralSubtrees) permitted.clone();
            }
            if (excluded != null) {
