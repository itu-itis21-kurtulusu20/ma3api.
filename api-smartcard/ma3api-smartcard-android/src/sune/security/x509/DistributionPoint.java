...
public class DistributionPoint {
    ...
    private sune.security.x509.GeneralNames fullName;
    private sune.security.x509.RDN relativeName;

    // reasonFlags or null
    private boolean[] reasonFlags;

    // crlIssuer or null
    private sune.security.x509.GeneralNames crlIssuer;
     ...
     */
    public DistributionPoint(sune.security.x509.GeneralNames fullName, boolean[] reasonFlags,
                             sune.security.x509.GeneralNames crlIssuer) {
     ...
     */
    public DistributionPoint(sune.security.x509.RDN relativeName, boolean[] reasonFlags,
                             sune.security.x509.GeneralNames crlIssuer) {
        ...
        while ((val.data != null) && (val.data.available() != 0)) {
            ...
            if (opt.isContextSpecific(TAG_DIST_PT) && opt.isConstructed()) {
                ...
                if (distPnt.isContextSpecific(TAG_FULL_NAME)
                    ...
                    fullName = new sune.security.x509.GeneralNames(distPnt);
                } else if (distPnt.isContextSpecific(TAG_REL_NAME)
                        && distPnt.isConstructed()) {
                    distPnt.resetTag(DerValue.tag_Set);
                    relativeName = new sune.security.x509.RDN(distPnt);
                } else {
                ...
                crlIssuer = new sune.security.x509.GeneralNames(opt);
            } else {
     ...
     */
    public sune.security.x509.GeneralNames getFullName() {
        return fullName;
    }
