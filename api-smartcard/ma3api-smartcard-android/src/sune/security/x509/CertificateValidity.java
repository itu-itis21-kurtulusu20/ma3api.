...
public class CertificateValidity implements CertAttrSet<String> {
    ...
    private void construct(sune.security.util.DerValue derVal) throws IOException {
        ...
        DerInputStream derIn = new sune.security.util.DerInputStream(derVal.toByteArray());
        sune.security.util.DerValue[] seq = derIn.getSequence(2);
        if (seq.length != 2)
            throw new IOException("Invalid encoding for CertificateValidity");

        if (seq[0].tag == sune.security.util.DerValue.tag_UtcTime) {
            notBefore = derVal.data.getUTCTime();
        } else if (seq[0].tag == sune.security.util.DerValue.tag_GeneralizedTime) {
            notBefore = derVal.data.getGeneralizedTime();
        } else {
            throw new IOException("Invalid encoding for CertificateValidity");
        }

        if (seq[1].tag == sune.security.util.DerValue.tag_UtcTime) {
            notAfter = derVal.data.getUTCTime();
        } else if (seq[1].tag == sune.security.util.DerValue.tag_GeneralizedTime) {
     ...
     */
    public CertificateValidity(sune.security.util.DerInputStream in) throws IOException {
    ...
    public void encode(OutputStream out) throws IOException {
        ...
        sune.security.util.DerOutputStream pair = new sune.security.util.DerOutputStream();
        ...
        seq.write(sune.security.util.DerValue.tag_Sequence, pair);
