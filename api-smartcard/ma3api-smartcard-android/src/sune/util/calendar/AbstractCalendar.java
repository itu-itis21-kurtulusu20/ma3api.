...
public abstract class AbstractCalendar extends CalendarSystem {
    ...
    public sune.util.calendar.Era getEra(String eraName) {
        if (eras != null) {
    ...
    public sune.util.calendar.Era[] getEras() {
        sune.util.calendar.Era[] e = null;
        if (eras != null) {
            e = new sune.util.calendar.Era[eras.length];
            System.arraycopy(eras, 0, e, 0, eras.length);
        }
        return e;
    }

    public void setEra(sune.util.calendar.CalendarDate date, String eraName) {
        if (eras == null) {
            return; // should report an error???
        }
        for (int i = 0; i < eras.length; i++) {
            sune.util.calendar.Era e = eras[i];
    ...
    public sune.util.calendar.CalendarDate getCalendarDate() {
        return getCalendarDate(System.currentTimeMillis(), newCalendarDate());
    }

    public sune.util.calendar.CalendarDate getCalendarDate(long millis) {
        return getCalendarDate(millis, newCalendarDate());
    }

    public sune.util.calendar.CalendarDate getCalendarDate(long millis, TimeZone zone) {
        sune.util.calendar.CalendarDate date = newCalendarDate(zone);
        return getCalendarDate(millis, date);
    }

    public sune.util.calendar.CalendarDate getCalendarDate(long millis, sune.util.calendar.CalendarDate date) {
        ...
        if (zi != null) {
            ...
            if (zi instanceof sune.util.calendar.ZoneInfo) {
                zoneOffset = ((sune.util.calendar.ZoneInfo)zi).getOffsets(millis, offsets);
            } else {
    ...
    public long getTime(sune.util.calendar.CalendarDate date) {
        ...
        if (zi != null) {
            ...
            if (date.isStandardTime()) {
                ...
                if (zi instanceof sune.util.calendar.ZoneInfo) {
                    ((sune.util.calendar.ZoneInfo)zi).getOffsetsByStandard(ms, offsets);
    ...
    protected long getTimeOfDay(sune.util.calendar.CalendarDate date) {
        long fraction = date.getTimeOfDay();
        if (fraction != sune.util.calendar.CalendarDate.TIME_UNDEFINED) {
            return fraction;
        }
    ...
    public long getTimeOfDayValue(sune.util.calendar.CalendarDate date) {
    ...
    public sune.util.calendar.CalendarDate setTimeOfDay(sune.util.calendar.CalendarDate cdate, int fraction) {
        if (fraction < 0) {
    ...
    protected abstract boolean isLeapYear(sune.util.calendar.CalendarDate date);

    public sune.util.calendar.CalendarDate getNthDayOfWeek(int nth, int dayOfWeek, sune.util.calendar.CalendarDate date) {
        sune.util.calendar.CalendarDate ndate = (sune.util.calendar.CalendarDate) date.clone();
        normalize(ndate);
    ...
    public static long getDayOfWeekDateOnOrBefore(long fixedDate, int dayOfWeek) {
        ...
        return fixedDate - sune.util.calendar.CalendarUtils.mod(fd, 7);
    }
     ...
     */
    protected abstract long getFixedDate(sune.util.calendar.CalendarDate date);
     ...
     * @see "AbstractCalendar.html fixed_date"
     */
    protected abstract void getCalendarDateFromFixedDate(sune.util.calendar.CalendarDate date,
                                                         long fixedDate);

    public boolean validateTime(sune.util.calendar.CalendarDate date) {
        int t = date.getHours();
        if (t < 0 || t >= 24) {
            return false;
        }
