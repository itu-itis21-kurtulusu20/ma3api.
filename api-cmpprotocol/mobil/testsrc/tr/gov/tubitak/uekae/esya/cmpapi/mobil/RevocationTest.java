...
public class RevocationTest {
    ...
    private static void client_RR_Example() throws Exception {
        ...
        try {
            ...
            if (!SignUtil.verify(SignatureAlg.RSA_SHA1, imzalanacak, signed,
                    KeyUtil.decodePublicKey(rACertificate.getSubjectPublicKeyInfo())) )
        ...
        IProtectionTrustProvider protectionTrustProvider = new ProtectionTrustProvider(
                Arrays.asList((IProtectionController) new ProtectionControllerWithSign(new SingleTrustedCertificateFinder(cACertificate))),
                new ProtectionGeneratorWithSign(signer, rACertificate));


        RevocationParamWithSerial revocationParam = new RevocationParamWithSerial(
        ...
        System.out.println("revocationParam.getPkiStatusInfo():" + AsnIO.getFormattedAsn(revocationParam.getPkiStatusInfo()));
