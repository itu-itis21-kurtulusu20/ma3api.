...
public class RevocationBuilder {
    ...
    public RevocationBuilder(ProtocolType protocolType) {
        ...
        if (protocolType != ProtocolType.REVOCATIONPROTOCOL)
            throw new RuntimeException("RevocationBuilder cannot be created for protocol:" + protocolType.getProtocolName());
    }
    ...
    public PKIBody createRevReqBody(RevDetails[] revDetailses) {
        ...
        if (!(reqType instanceof PKIMessageType.RevReqType))
            throw new RuntimeException("RevReqBody cannot be created for protocol:" + protocolType.getProtocolName());
...
/*                    if (mRecResponse.keyPairHist != null) {
     ...
     public List<Pair<CertId, PKIStatusInfo>> verifyRevRepMessage(PKIBody responseBody) throws CMPProtocolException {
         ...
         if (pkiMessageType != PKIMessageType.RP)
             throw new CMPProtocolException(EPKIFailureInfo.incorrectData, "Invalid PKI Body, must be " + PKIMessageType.RP + ", But it is:" + pkiMessageType);
         ...
         if(revRepContent.status.elements.length != revocationParams.size())
          throw new CMPProtocolException(EPKIFailureInfo.incorrectData, "Invalid Number of Status in Response, Expected: " 
