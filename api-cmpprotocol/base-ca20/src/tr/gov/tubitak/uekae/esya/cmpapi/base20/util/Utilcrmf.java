...
//        AlgorithmIdentifier asimAlg = aEncVal.keyAlg;
        ...
        Cipher decryptor = new GNUCryptoProvider().getDecryptor(sa.first());
        BufferedCipher simKrip = new BufferedCipher(decryptor);
...
/*        try {
    ...
    public static PrivateKey decryptPrivateKey(byte[] protocolEncKeyBytes,EncryptedValue encryptedPrivateKey) throws Exception {
        ...
        try {
            ...
            try {
                retKey = decodePrivateKey(asymmetricAlg, protocolEncKeyBytes);
            } catch (Exception e) {
                retKey = decodePrivateKey(asymmetricAlg, new EPrivateKeyInfo(protocolEncKeyBytes).getEncoded());
            }
        } catch (Exception ex) {
            ex.printStackTrace();
            throw ex;
    ...
    private static PrivateKey decodePrivateKey(AsymmetricAlg asymmetricAlg, byte[] privateKeyBytes) throws CryptoException {
