...
public class X509CVCParam extends BaseCertificationParam {
    ...
    protected static Logger logger = LoggerFactory.getLogger(X509CVCParam.class);
    ...
    public void setAskidaUret(boolean askidaUret){
        try {
            ...
            logger.error("Error in X509CVCParam", e);
        }
    }
    ...
    public void addSpecificOperations(ECertReqMsg certReqMsg) {
        ...
        if (sdoHash != null) {
            try {
                Utilcrmf.istegeASN1TypeEkle(certReqMsg.getCertRequest().getObject(), new Asn1OctetString(sdoHash.getEncoded()), EESYAOID.oid_ESYA_SDO);
            } catch (Exception aEx) {
                throw new ESYARuntimeException("Error while encoding EsyaSdoHash:" + aEx.getMessage(), aEx);
            }
        }

        if (cardSerialNumber != null) {
            try {
                Utilcrmf.istegeASN1TypeEkle(certReqMsg.getCertRequest().getObject(), new Asn1OctetString(cardSerialNumber.getEncoded()), EESYAOID.oid_ESYA_CardSerialNumber);
            } catch (Exception aEx) {
                throw new ESYARuntimeException("Error while encoding EsyaSdoHash:" + aEx.getMessage(), aEx);
            }
        }
        if (sertifikaTalepNo != null) {
            try {
                ...
                throw new ESYARuntimeException("Error while encoding SertifikaTalepNo:" + aEx.getMessage(), aEx);
            }
        }

        if (yeniSertifikaDurumu != null) {
            try {
                Utilcrmf.istegeASN1TypeEkle(certReqMsg.getCertRequest().getObject(), new Asn1OctetString(yeniSertifikaDurumu.getEncoded()), EESYAOID.oid_cmpYeniSertifikaDurumu);
            } catch (Exception aEx) {
                throw new ESYARuntimeException("Error while encoding cmpYeniSertifikaDurumu:" + aEx.getMessage(), aEx);
            }
        }
