...
// ----------------------------------------------------------------------------
// $Id: Util.java,v 1.1 2004/11/02 11:39:43 serdar Exp $
//
...
public class Util {
    ...
    * @param bLength desired byte length
    * @return
    */
   public static byte [] toBytes(BigInteger x, int bLength){
	   byte[] bytes = x.toByteArray();
       
	 //Trim the bytes from the beginning of array
       if (bLength < bytes.length)
       {
           byte[] tmp = new byte[bLength];
       
           System.arraycopy(bytes, bytes.length - tmp.length, tmp, 0, tmp.length);
           
           return tmp;
       }
       //Fill with zero to the beginning of array
       else if (bLength > bytes.length)
       {
           byte[] tmp = new byte[bLength];
