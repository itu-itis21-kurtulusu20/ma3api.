...
// ----------------------------------------------------------------------------
// $Id: EMSA_PSS.java,v 1.1 2004/11/02 11:39:42 serdar Exp $
//
...
public class EMSA_PSS implements Cloneable {
   ...
   public byte[] encode(byte[] mHash, int emBits, byte[] salt) {
      ...
      IMessageDigest hashZ = (IMessageDigest) hash.clone();
      MGF1 mgf = new MGF1(hashZ);
      mgf.init(H, null);
      byte[] dbMask = mgf.generateBytes(emLen - hLen - 1);
