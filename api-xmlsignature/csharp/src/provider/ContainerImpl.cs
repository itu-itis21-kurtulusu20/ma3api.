...
namespace tr.gov.tubitak.uekae.esya.api.xmlsignature.provider
{
    ...
    public class ContainerImpl : AbstractSignatureContainer
    {
        ...
        {
            if (!(signature is SignatureImpl))
                throw new SignatureRuntimeException("Unknown CMS Signature impl! "+signature.GetType());
        ...
        {
            try {
                //Document doc = new StreamingDocument(stream, xmlContext.BaseURIStr, "text/xml", null);

                // java'da doc.bytes ile dokuman olusturuluyordu, burda create byte[]almadigi icin stream yaptim
                //MemoryStream ms = new MemoryStream(doc.Bytes);
                
                XmlDocument domDocument = new XmlDocument();
                XmlReader reader = XmlReader.Create(/*ms*/stream);
                domDocument.PreserveWhitespace = true;
                domDocument.Load(reader);

                XmlElement xmlElement = domDocument.DocumentElement;
                if(xmlElement.LocalName.Equals("envelope"))
                {
                    Document doc = new StreamingDocument(stream, xmlContext.BaseURIStr, "text/xml", null);
                    signedDocument = new SignedDocument(doc, xmlContext);
                }
                else if (xmlElement.LocalName.Equals(Constants.TAG_SIGNATURE))
                {
                    XMLSignature signature = new XMLSignature(domDocument.DocumentElement, xmlContext);
                    xmlContext.Document = null;
                    signedDocument = new SignedDocument(xmlContext);
                    signedDocument.addSignature(signature);
                }

                foreach(XMLSignature s in  signedDocument.getRootSignatures()){
        ...
        {
            if (signedDocument.RootSignatureCount == 1)
        ...
        public static void main(String[] args)
        {
            ...
            // java'da byte signed yani -128:127 arasi, c#'ta ise unsigned yani 0:255 arasi
