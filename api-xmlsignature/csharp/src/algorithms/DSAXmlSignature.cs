...
namespace tr.gov.tubitak.uekae.esya.api.xmlsignature.algorithms
{
	...
	/// date: Aug 26, 2009
	/// </summary>
	public class DSAXmlSignature : BaseXmlSignatureAlgorithm
	{
		public DSAXmlSignature(SignatureAlg aSignatureAlg) : base(aSignatureAlg)
		{
		}

		public override byte[] sign()
		{
			byte[] asn1Bytes = base.sign();
			return convertASN1toXMLDSIG(asn1Bytes);
		}

		public override bool verify(byte[] aSignatureValue)
		{
			return base.verify(convertXMLDSIGtoASN1(aSignatureValue));
		}

		/// <summary>
		/// Converts an ASN.1 DSA value to a XML Signature DSA Value.
		/// <p/>
		/// XML Signature requires the core BigInteger values. </summary>
		/// <param name="asn1Bytes"> </param>
		/// <returns> the decode bytes </returns>
		...
		/// <p/>
		/// the XML Signature requires the core BigInteger values. </summary>
		/// <param name="xmldsigBytes"> </param>
		/// <returns> the encoded ASN.1 bytes </returns>
		...
		/// </summary>
		/// <param name="asn1Bytes"> to convert XMLDSİg version </param>
		/// <returns> the decode bytes
		/// </returns>
		/// <exception cref="XMLSignatureException"> if invalid format is found </exception>
		/// <seealso cref= <A HREF="http://www.w3.org/TR/xmldsig-core/#dsa-sha1">6.4.1 DSA</A> </seealso>
        private byte[] convertASN1toXMLDSIG(byte[] asn1Bytes)
		{
			...
			Array.Copy(asn1Bytes, (4 + rLength) - i, xmldsigBytes, 20 - i, i);
			Array.Copy(asn1Bytes, (6 + rLength + sLength) - j, xmldsigBytes, 40 - j, j);
		...
		private static byte[] convertXMLDSIGtoASN1(byte[] xmldsigBytes)
		{
			...
			Array.Copy(xmldsigBytes, 20 - i, asn1Bytes, (4 + j) - i, i);

			asn1Bytes[4 + j] = 2;
			asn1Bytes[5 + j] = (byte) l;

			Array.Copy(xmldsigBytes, 40 - k, asn1Bytes, (6 + j + l) - k, k);

			return asn1Bytes;
		}

		public class DSAWithSHA1 : DSAXmlSignature
		{
			public DSAWithSHA1() : base(SignatureAlg.DSA_SHA1)
			{
			}
		}
