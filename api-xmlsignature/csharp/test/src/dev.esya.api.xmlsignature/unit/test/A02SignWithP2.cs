...
namespace tr.gov.tubitak.uekae.esya.api.xmlsignature.unit.test
{
    ...
    class SignWithP2
    {
        ...
        public void a_createEnveloping()
        {
            ...
            POLICY_FILE_PATH = ROOTDIR + "\\docs\\config\\policy\\" + UnitTestParameters.POLICY_FILE_NAME2;
            try
            {
                Context context = new Context(BASEDIR);
                /*OfflineResolver policyResover = new OfflineResolver();
                policyResover.register("2.16.792.1.61.0.1.5070.3.1.1", POLICY_FILE_PATH, "text/xml");
                context.addExternalResolver(policyResover);*/
                context.addExternalResolver(POLICY_RESOLVER);
                XMLSignature signature = new XMLSignature(context);
                signature.setPolicyIdentifier(UnitTestParameters.OID_POLICY_P2, "Kısa Dönemli ve SİL Kontrollü Güvenli Elektronik İmza Politikası", POLICY_FILE_PATH);
