...
namespace tr.gov.tubitak.uekae.esya.api.xmlsignature.unit.test
{
    ...
    class SignWithP3
    {
        ...
        public void a_createEnveloping()
        {
            ...
            POLICY_FILE_PATH = ROOTDIR + "\\docs\\config\\policy\\" + UnitTestParameters.POLICY_FILE_NAME3;
            try
            {
                Context context = new Context(BASEDIR);
                context.addExternalResolver(POLICY_RESOLVER);
                XMLSignature signature = new XMLSignature(context);
                signature.setPolicyIdentifier(UnitTestParameters.OID_POLICY_P3, "Uzun Dönemli ve SİL Kontrollü Güvenli Elektronik İmza Politikası", POLICY_FILE_PATH);
