...
namespace tr.gov.tubitak.uekae.esya.api.cmssignature.validation.check
{
     ...
     *
     */
    public class ArchiveTimeStampV2AttrChecker : BaseArchieveTimeStampAttrChecker
    {
        //@Override
        protected override bool _check(Signer aSigner, CheckerResult aCheckerResult)
        {
            ...
            aCheckerResult.setCheckerName(Msg.getMsg(Msg.ARCHIVE_TIMESTAMP_V2_ATTRIBUTE_CHECKER), typeof(ArchiveTimeStampV2AttrChecker));
            ...
            try
            {
                ...
                if (tsAttrs.Count == 0)
                {
                    aCheckerResult.addMessage(new ValidationMessage(Msg.getMsg(Msg.NO_ARCHIVE_TSA_V2_IN_SIGNEDDATA)));
            ...
            {
                aCheckerResult.addMessage(new ValidationMessage(Msg.getMsg(Msg.ARCHIVE_TSA_V2_DECODE_ERROR), aEx));
                return false;
            }
            ...
            if (!allResult)
            {
                aCheckerResult.addMessage(new ValidationMessage(Msg.getMsg(Msg.ARCHIVE_TSA_V2_CHECK_UNSUCCESSFUL)));
            }
            else
            {
                aCheckerResult.addMessage(new ValidationMessage(Msg.getMsg(Msg.ARCHIVE_TSA_V2_CHECK_SUCCESSFUL)));
        ...
        public override bool checkOneTimeStampAttr(EAttribute attr, Signer aSigner, CheckerResult aCheckerResult)
        {
                ...
                {
                    aCheckerResult.addMessage(new ValidationMessage(Msg.getMsg(Msg.ARCHIVE_TSA_V2_CHECK_UNSUCCESSFUL), aEx));
                    return false;
                }
