...
public class ArchiveTimeStampAttrChecker extends BaseArchiveTimeStampAttrChecker
{
	...
	{
		aCheckerResult.setCheckerName(CMSSignatureI18n.getMsg(E_KEYS.ARCHIVE_TIMESTAMP_ATTRIBUTE_CHECKER), ArchiveTimeStampAttrChecker.class);
		...
		try
		{
			...
			if(tsAttrs.isEmpty())
			{
				aCheckerResult.addMessage(new ValidationMessage(CMSSignatureI18n.getMsg(E_KEYS.NO_ARCHIVE_TSA_IN_SIGNEDDATA)));
		...
		{
			aCheckerResult.addMessage(new ValidationMessage(CMSSignatureI18n.getMsg(E_KEYS.ARCHIVE_TSA_DECODE_ERROR),aEx));
			return false;
		}
		...
		if(!allResult)
		{
			aCheckerResult.addMessage(new ValidationMessage(CMSSignatureI18n.getMsg(E_KEYS.ARCHIVE_TSA_CHECK_UNSUCCESSFUL)));
		}
		else
		{
			aCheckerResult.addMessage(new ValidationMessage(CMSSignatureI18n.getMsg(E_KEYS.ARCHIVE_TSA_CHECK_SUCCESSFUL)));
	...
	public boolean checkOneTimeStampAttr(EAttribute attr, Signer aSigner, CheckerResult aCheckerResult) {
		...
		{
			aCheckerResult.addMessage(new ValidationMessage(CMSSignatureI18n.getMsg(E_KEYS.ARCHIVE_TSA_CHECK_UNSUCCESSFUL),aEx));
			return false;
		}
