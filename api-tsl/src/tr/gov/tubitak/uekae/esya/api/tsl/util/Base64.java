...
public class Base64 {
	...
	protected static Logger logger = LoggerFactory.getLogger(Base64.class);
	 ...
	 */
	private static final byte[] ALPHABET = { (byte) 'A', (byte) 'B',
	 ...
	 * @throws TSLException
	 */
	public static final byte[] decode(final String s) throws TSLException {
		byte[] bytes = null;
		try {
			bytes = s.getBytes("US-ASCII");
		} catch (UnsupportedEncodingException x) {
			// not expected
			logger.error("Error in Base64", x);
			throw new TSLException("Unsupported encoding", x);
		}
		...
		for (i = off; i < off + len; i++) {
			...
			if (sbiDecode >= WHITE_SPACE_ENC) { // White space, Equals sign or
				...
				throw new IllegalArgumentException(
						"Illegal BASE-64 character at #" + i + ": " + src[i]
								+ "(decimal)");
			}
		}
	...
	private static final byte[] encode3to4(final byte[] src, final int sOffset,
			final int numBytes, final byte[] dest, final int dOffset) {
		// 1 2 3
	...
	private static final int decode4to3(final byte[] src, final int sOffset,
		...
		try { // Example: DkLE
			...
			logger.warn("Warning in Base64", x);
			if (DEBUG && debuglevel > 8) {
				debug("" + src[sOffset] + ": " + (DECODABET[src[sOffset]]));
				debug("" + src[sOffset + 1] + ": "
						+ (DECODABET[src[sOffset + 1]]));
				debug("" + src[sOffset + 2] + ": "
						+ (DECODABET[src[sOffset + 2]]));
				debug("" + src[sOffset + 3] + ": "
						+ (DECODABET[src[sOffset + 3]]));
			}
			return -1;
		}
	}
