...
{
	/*
	QLibrary lib("ntdll.dll");
	if (lib.load())
	{		
		pfnNtQueryInformationProcess func = (pfnNtQueryInformationProcess)lib.resolve("NtQueryInformationProcess");
		if (func == NULL)
		{
			return NULL;
		}
	...
	return NULL;
	*/
	HMODULE hNtDll = LoadLibrary((LPCWSTR)QString("ntdll.dll").utf16());
...
//				}
		...
		if(spi.dwPID == 4) {
			LPCTSTR str=(LPCTSTR)("%SystemRoot%\\System32\\ntoskrnl.exe");
