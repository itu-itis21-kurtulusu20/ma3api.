...
public class CMPPBMac
{
      ...
      * @throws CryptoException init sirasinda cikabilecek exception
      *
      ...
      * @throws CryptoException init sirasinda cikabilecek exception
      *
     public CMPPBMac(PBMParameter aParam,String aPassword)
         throws CryptoException
     {
          _init(aParam,aPassword);
     }

     private void _init(PBMParameter aParam,String aPassword)
         throws CryptoException
     {
          byte[] salt;
          DigestAlg ozet;
          ...
          salt = aParam.salt.value;
          if(salt.length > MAXSALTLEN)
               throw new CryptoException("Salt cok uzun : "+salt.length);
          ozet = DigestAlg.fromOID(aParam.owf.algorithm.value);
          iterationCount = (int)aParam.iterationCount.value;
          if(iterationCount > MAXITERATIONLEN)
               throw new CryptoException("Iteration count cok uzun : "+iterationCount);
          if(!Arrays.equals(aParam.mac.algorithm.value,MACAlg.HMAC_SHA1.getOID()))
               throw new CryptoException("Mac algoritmasi bilinmiyor  "+aParam.mac.algorithm);
         ...
         throws CryptoException
     {
          ...
          {
               LOGCU.fatal("Buraya hic gelmemeli. Charset desteklenmiyor.");
          ...
          {
               LOGCU.fatal("Bu durum henuz implement edilmedi.");
          ...
          {
               LOGCU.fatal("Buraya hic gelmemeli. Mac init edilemedi.");
